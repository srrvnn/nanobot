...
class AgentLoop:
  ...
  def _register_default_tools(self) -> None:
    ...
    self.tools.register(HexEncodeTool())
    ...
    discover_kernel_tools(self.tools)
  ...
  def _set_tool_context(
    ...
    for name in self.tools.tool_names:
      ...
      if not tool:
        continue
      if name in ("message", "spawn", "cron") and hasattr(tool, "set_context"):
        tool.set_context(
      ...
      if isinstance(tool, AndroidKernelToolWrapper):
        if user_request:
          tool.set_user_request(user_request)
  ...
  async def _run_agent_loop(
    ...
    while iteration < self.max_iterations:
      ...
      if response.has_tool_calls:
        ...
        tool_call_dicts = [
            {
                ...
                "function": {
                    "name": tc.name,
                    "arguments": json.dumps(tc.arguments, ensure_ascii=False) if not isinstance(tc.arguments, str) else tc.arguments,
                },
            }
        ...
        for tool_call in response.tool_calls:
          ...
          if isinstance(tool, AndroidKernelToolWrapper):
            # Extract rationale from reasoning content or thinking blocks
            rationale = response.reasoning_content
            if not rationale and response.thinking_blocks:
              # Try to extract text from thinking blocks
              parts = []
              for block in response.thinking_blocks:
                if isinstance(block, dict):
                  if block.get("type") == "text":
                    ...
                    parts.append(str(block["thought"]))
              rationale = " ".join(parts).strip()

            # Fallback to content or a descriptive default
            rationale = (
                rationale
                or response.content
                or f"Required to {tool.description.lower()}"
            )
            tool.set_rationale(rationale)

          result = await self.tools.execute(tool_call.name, tool_call.arguments)

          # Check for base64 image in result (e.g. from computer_control_screenshot)
          image_data = None
          try:
            if (
                tool_call.name == "computer_control_screenshot"
              ...
              if "base64_image" in data:
                ...
                # Save image to file for user download/viewing
                import base64
                ...
                logger.info("Saved screenshot to {}", file_path)

                # Replace huge base64 string in text history to save tokens/sanity
                # Provide path so agent can use it with MessageTool if desired
                result = json.dumps({
                    "base64_image": "<image_data_hidden>",
                    "path": f"screenshots/{filename}",
                    "note": (
                        "Image saved to file and injected into context as an"
                        " image block."
                    ),
                })
          except Exception as e:
            logger.error("Error processing screenshot: {}", e)
          ...
          if image_data:
            # Inject the screenshot as a new user message so the model can see it
            logger.info(
                "Injecting screenshot from {} into context", tool_call.name
            )
            messages.append({
                "role": "user",
                "content": [
                    {
                        "type": "text",
                        "text": f"Screenshot output from {tool_call.name}:",
                    },
                    {
                        "type": "image_url",
                        "image_url": {
    ...
    if final_content is None and iteration >= self.max_iterations:
      ...
      final_content = (
          ...
          f" ({self.max_iterations}) without completing the task. You can try"
  ...
  async def _handle_stop(self, msg: InboundMessage) -> None:
    ...
    content = (
        f"⏹ Stopped {total} task(s)." if total else "No active task to stop."
    )
          ...
          if snapshot:
            ...
            if not await self._consolidate_memory(temp, archive_all=True):
              return OutboundMessage(
                  ...
                  chat_id=msg.chat_id,
                  content=(
                      "Memory archival failed, session not cleared. Please try"
                      " again."
                  ),
              )
      except Exception:
